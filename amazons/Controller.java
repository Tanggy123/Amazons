...
final class Controller {
        ...
        while (_playing) {
            ...
            if (_winner == null) {
                if (_board.winner() != null) {
                    reportNote(_board.winner().toName() + " wins.");
     ...
     * Command "manual C".
     * @param p Matcher.*/
    private void doManual(Matcher p) {
        String piece = p.group(2);
     ...
     * @param mov Matcher
     */
    private void doMove(Matcher mov) {
        Move m = Move.mv(mov.group());
        if (!_board.isLegal(m)) {
            throw error("Illegal move. Please try again.");
        }
        _board.makeMove(m);
    }
